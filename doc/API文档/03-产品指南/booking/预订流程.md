...
当 Atlas 是您主要的搜索层时，使用[标准预订流程](/api-wen-dang/product-guides/booking/booking-flows/standard-booking-flow.md)。

从 `search.do` 开始。

然后在 `order.do` 之前继续调用 `verify.do`。

当行程已知或需要独立价格检查时，使用[获取报价流程](/api-wen-dang/product-guides/booking/booking-flows/get-offer-flow.md)。

从 `getOffers.do` 开始。

然后使用 `OfferId` 继续调用 `order.do`。

当您需要更广泛的报价可见性且可以在创建订单后立即开始支付时，使用[履约流程](/api-wen-dang/product-guides/booking/booking-flows/fulfillment-flow.md)。
