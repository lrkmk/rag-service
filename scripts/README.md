...
`diff-check` 每次跑完会把结果写成 `doc/<corpus>/.crawl-status.json`（`new`/`changed`/`unchanged`/`unmatched`，已 gitignore，只是当次检查结果的标记，不是持久状态）——切片时只处理 `changed` 里列出的路径（以及已经手动 `fetch`+归位的 `new`），不用把整个语料库重新切一遍。这个文件跑完 `diff-check` 就该马上用，隔太久再看会过期（站点内容可能又变了）。
...
`sentence-transformers` 用于本地跑 embedding 模型，当前用的是 `BAAI/bge-m3`（多语言，2026-07-22 从纯中文的 `BAAI/bge-large-zh-v1.5` 切过来的）。Chroma 自带的默认 embedder 是英文模型，对中文效果差，所以一直用 BGE 系列；中间在"纯中文模型 vs 多语言模型"上反复过一次——最早用多语言、后来因为语料几乎全是中文换成纯中文 BGE、现在又换回多语言的 bge-m3，触发点是产品介绍语料里混杂中文正文+裸英文专有名词（航司名、产品术语）时，纯中文模型匹配英文词效果很差（实测"IndiGo"这类查询距离 ~0.86，同一 chunk 换自然语言中文查询只要 ~0.53），bge-m3 对这种混合场景明显更好，纯中文场景也没有变差（上线前用 `eval/*.jsonl` 那套 Recall@k/MRR 评测集在三个语料库上都测过）。如果你更想用云端 embedding（OpenAI/智谱等），把 `EMBEDDER` 换成 `embedding_functions.OpenAIEmbeddingFunction(...)` 之类即可，其余逻辑不用动。

**换模型时注意不对称前缀的差异**：`bge-large-zh-v1.5` 是不对称检索模型，建议给*查询*文本加指令前缀（"为这个句子生成表示以用于检索相关文章："），但*文档*不用加——这也是当时 `search/query_example.py`/`search/rag_search.py` 不用 Chroma 的 `query_texts=` 自动 embedding（那样查询和文档会用同一套无前缀逻辑，前缀效果就丢了），而是手动 `encode()` 查询文本后传 `query_embeddings=` 的原因。`bge-m3` 是按对称/免指令检索训练的，加前缀反而更差（同样的"IndiGo"测试，加了前缀从 0.55 变差到 0.62），所以 `rag_search.py` 里 `QUERY_INSTRUCTION` 现在是空字符串。如果以后再换别的模型，先确认它是对称还是不对称检索，别想当然照抄现在的空前缀。
